...
class SubarrayQueryProcessor:
    def __init__(self, array_elements):
        ...
        self.prefix_sums = self._compute_prefix_sums(array_elements)

    def _compute_prefix_sums(self, array_elements):
        prefix_sums = [0]
        running_sum = 0
...
def read_positive_integer(prompt_message):
    while True:
...
def read_array_of_positive_integers(prompt_message, expected_length):
    while True:
        user_input = input(prompt_message)
        array_elements = []
        current_number_str = ""
...
def main():
    ...
    number_of_elements = read_positive_integer("Enter the number of elements in the array: ")
    number_of_queries = read_positive_integer("Enter the number of queries you want to perform: ")

    array_elements = read_array_of_positive_integers(
        f"Enter {number_of_elements} elements of the array separated by spaces: ",
        number_of_elements
    )

    query_processor = SubarrayQueryProcessor(array_elements)
    ...
    for query_number in range(1, number_of_queries + 1):
        while True:
            ...
            if left_index < 1 or right_index > number_of_elements or left_index > right_index:
                print(
                    f"Invalid indices. Please enter values between 1 and {number_of_elements} with left_index <= right_index."
                )
                continue
            break

        floor_mean_result = query_processor.calculate_floor_mean(left_index, right_index)
